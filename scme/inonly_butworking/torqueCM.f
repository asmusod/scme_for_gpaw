      subroutine torqueCM(dpole, qpole, opole, hpole, d1v, d2v, d3v, d4v
     $     , nM, tau)  
...
c     High order derivatives of the potential
      ...
      real*8 d4v(3,3,3,3,maxCoo/3)

      integer nM
      real*8 tau(3,maxCoo/3), asy
...
c     Torque on the octopoles
         do j = 1, 3
            do i = 1, 3
               t(1) = t(1) - 3.d0 / 15.d0 * (
     $              opole(2,i,j,n)*d3v(3,i,j,n) - 
     $              opole(3,i,j,n)*d3v(2,i,j,n))
               t(2) = t(2) - 3.d0 / 15.d0 * (
     $              opole(3,i,j,n)*d3v(1,i,j,n) - 
     $              opole(1,i,j,n)*d3v(3,i,j,n))
               t(3) = t(3) - 3.d0 / 15.d0 * (
     $              opole(1,i,j,n)*d3v(2,i,j,n) - 
     $              opole(2,i,j,n)*d3v(1,i,j,n))  
            end do
         end do

c     Torque on the hexadecapoles
         do k = 1, 3
            do j = 1, 3
               do i = 1, 3
                  t(1) = t(1) - 4.d0 / 105.d0 * (
     $                 hpole(2,i,j,k,n)*d4v(3,i,j,k,n) -
     $                 hpole(3,i,j,k,n)*d4v(2,i,j,k,n))
                  t(2) = t(2) - 4.d0 / 105.d0 * (
     $                 hpole(3,i,j,k,n)*d4v(1,i,j,k,n) -
     $                 hpole(1,i,j,k,n)*d4v(3,i,j,k,n))
                  t(3) = t(3) - 4.d0 / 105.d0 * (
     $                 hpole(1,i,j,k,n)*d4v(2,i,j,k,n) -
     $                 hpole(2,i,j,k,n)*d4v(1,i,j,k,n))
               end do
            end do
         end do
