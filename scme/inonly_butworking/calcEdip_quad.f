      subroutine calcEdip_quad(rCM, dpole, qpole, nM, NC, a, a2, uD, uQ,
     $     eT, dEdr, rMax2, iSlab)
      ...
      real*8 rCM(3,maxCoo/3), dpole(3,maxCoo/3), qpole(3,3,maxCoo/3),
     $     a(3), a2(3)
      real*8 dEdr(3,3,maxCoo/3), uD, uQ, eT(3,maxCoo/3)

      real*8 re(3), r1, r2, r3, r5, r7, eD(3), eq(3), dr(3), dEdr1(3,3),
     $     u, rMax2, swFunc, dSdr
...
c     Dipole Field
                     call dField(dr, r2, r3, r5, eD, dpole, u,
     $                    dEdr1, j) 
                     uD = uD + u
                     do k = 1, 3
...
c     Quadrupole Field
                     call qField(dr, r2, r5, r7, eq, qpole, u,
     $                    dEdr1, j)
                     uQ = uQ + u
                     do k = 1, 3
...
c----------------------------------------------------------------------+
c     Calculate the dipolar field and its derivative                   |
c----------------------------------------------------------------------+
      ...
      real*8 dpole(3,maxCoo/3), eD(3), dr(3), r2, r3, r5, dEdr(3,3), u
...
c----------------------------------------------------------------------+
      ...
      real*8 qpole(3,3,maxCoo/3)

      integer i, j, m
      real*8 dr(3), r2, r5, r7, u, dEdr(3,3), eq(3)
      real*8 v(3), rQr
     ...
     $           * dr(i) + v(i) * dr(j)) - 35.d0 * rQr * dr(i) * dr(j) /
     $           r2) / r7
            if (i.eq.j) then
