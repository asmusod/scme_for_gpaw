...
c----------------------------------------------------------------------+
c     Routine to fix molecules broken due to the periodic boundary     |
c     conditions - ORIGINAL                                            |
c----------------------------------------------------------------------+
c      subroutine recoverMolecules(raOri, ra, nH, nO, a, a2)
c
c      implicit real*8 (a-h,o-z)
c      include '../commonblks/parameters.cmn'
c
c      real*8 raOri(maxCoo), a(3), a2(3), ra(maxCoo), dist
c      integer nH,nO
c
c      do i = 1, nO
c         do l = 1, 3
c            do n = 1, 2
c               index = l+3*(n-1)+6*(i-1)
c               dist = raOri(index) - raOri(l+3*(i-1+nH))
c               if (dist .gt. a2(l)) then
c                  ra(index) = raOri(index) - a(l)
c               elseif(dist .lt. -a2(l)) then
c                  ra(index) = raOri(index) + a(l)
c               else
c                  ra(index) = raOri(index)
c               end if
c           end do
c            ra(l+3*(i-1+nH)) = raOri(l+3*(i-1+nH))
c         end do
c     end do
c      return
c     end
...
c----------------------------------------------------------------------+
c     Add all the fields - QM/MM SCME 2015 added eQM field from edens  |
c----------------------------------------------------------------------+
      subroutine addFieldsQM(eH, eD, eTQM, eQM, nM)
      ...
      do i = 1, nM
         do j = 1, 3
            eTQM(j,i) = eH(j,i) + eD(j,i) + eQM(j,i)
         end do
      end do
      return
      end
